...
graph TD
    subgraph Initialization [初始化阶段]
        InitStart(应用启动) --> InitService[初始化 SemanticSearchService]
        InitService --> LoadWasm[加载 jieba-wasm 分词器]
        InitService --> InitDB[初始化 IndexedDB]
        InitDB --> LoadVocab[加载词汇表]
        InitDB --> LoadVectors[加载向量索引]
        LoadWasm & LoadVocab & LoadVectors --> Ready(服务就绪)
    end

    subgraph Indexing [索引流程]
        InputPrompts(输入提示词) --> Tokenize[jieba 分词处理]
        Tokenize --> BuildVocab{是否构建新词汇表?}
        BuildVocab -->|批量索引| CalcIDF[统计词频 & 计算 IDF]
        BuildVocab -->|增量索引| UseVocab[使用现有词汇表]
        CalcIDF --> GenVector[生成 TF-IDF 向量]
        UseVocab --> GenVector
        GenVector --> Normalize[L2 归一化]
        Normalize --> Persist[存储到 IndexedDB]
        Persist --> UpdateMem[更新内存索引]
    end

    subgraph Searching [搜索流程]
        UserQuery(用户查询) --> QueryTokenize[对查询分词]
        QueryTokenize --> ExtractKw[提取关键词]
        QueryTokenize --> QueryVector[生成查询向量]
        QueryVector --> CalcSim[计算余弦相似度]
        CalcSim --> Filter[阈值过滤]
        Filter --> Rank[排序取 Top-K]
        Rank --> ReturnRes(返回结果)
    end
